...
<div class="card card-custom">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">Sıkça Sorulan Sorular(SSS)</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm">Aşağıda sistemde yer alan tüm SSS'leri
        ...
        <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
            ...
            <tbody>
                @foreach ($sss as $s)
                    <tr>
                        ...
                        <td class="pr-0 text-right">
                            ...
                            <a href="#" data-toggle="modal" data-target="#delete_{{ $s->faq_guid }}"
                    ...
                    <div class="modal" id="delete_{{ $s->faq_guid }}" tabindex="-1" role="dialog">
                        <form action="{{ route('admin.faq_delete') }}" method="POST">
                            @csrf
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">SSS Sil</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <p>{{ $s->title }} adlı SSS'ı silmek istediğinize emin misiniz?</p>
                                    </div>
                                    <input type="hidden" name="faq_guid" value="{{ $s->faq_guid }}">
                    ...
                    <div class="modal fade" id="edit_{{ $s->faq_guid }}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <form action="{{ route('admin.faq_update') }}" method="POST">
                            @csrf
                            <input type="hidden" name="faq_guid" value="{{ $s->faq_guid }}">
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">SSS Düzenle</h5>
                ...
                <div class="modal fade" id="{{ $data['button']['id'] }}" tabindex="-1" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <form action="{{ route('admin.faq_add') }}" method="POST">
                        @csrf
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
