...
<div class="card card-custom">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">Kütüphane Kategorileri</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm">Aşağıda sistemde yer alan kütüphane
                kategorilerini
                görebilirsiniz.</span>
        </h3>
    </div>
            ...
            <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
                ...
                <tbody>
                    @foreach ($category as $c)
                        <tr>
                            ...
                            <td class="pr-0 text-right">
                                <a href="#" data-toggle="modal"
                                    data-target="#modal_{{ $c->library_category_guid }}"
                                ...
                                <a href="#" data-toggle="modal"
                                    data-target="#delete_{{ $c->library_category_guid }}"
                        ...
                        <div class="modal fade" id="modal_{{ $c->library_category_guid }}" tabindex="-1" role="dialog"
                            aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <form action="{{ route('admin.library_category_update') }}" method="POST"
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" value="{{ $c->title }}"
                                                    name="title" required>
                                                <input type="hidden" name="library_category_guid"
                                                    value="{{ $c->library_category_guid }}">
                                            </div>
                                            <div class="form-group">
                                                <label for="order" class="col-form-label">Görüntülenme Sırası</label>
                                                <input type="number" class="form-control"
                                                    value="{{ $c->order }}" name="order" name="order"
                        ...
                        <div class="modal" id="delete_{{ $c->library_category_guid }}" tabindex="-1"
                            role="dialog">
                            <form action="{{ route('admin.library_category_delete') }}" method="POST">
                                @csrf
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        ...
                                        <div class="modal-body">
                                            <p><b>{{ $c->title }}</b> kategorisini silmek istediğinize emin
                                                misiniz?</p>
                                        </div>
                                        <input type="hidden" name="library_category_guid"
                                            value="{{ $c->library_category_guid }}">
                    ...
                    <div class="modal fade" id="{{ $data['button']['id'] }}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <form action="{{ route('admin.library_category_add') }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Kütüphane Kategorisi Ekle</h5>
                                        <button type="button" class="close" data-dismiss="modal"
                                            aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
    ...
    @if ($category->lastPage() > 1)
        <div class="card-footer">
            {{ $category->links('backend.modules.global.paginator', ['paginator' => $category]) }}
        </div>
    @endif
</div>
