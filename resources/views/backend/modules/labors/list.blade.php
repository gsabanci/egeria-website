...
<div class="card card-custom">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">Hizmetler</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm">Aşağıda sistemde yer alan bütün hizmetleri
                görebilirsiniz.</span>
        </h3>
    </div>
    <!--end::Header-->
            ...
            <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
                ...
                <tbody>
                    @foreach ($labors as $labor)
                        <tr>
                            ...
                            <td class="pr-0 text-right">
                                <a href="" data-toggle="modal" data-target="#modal_{{ $labor->labor_guid }}"
                                ...
                                <a href="#" data-toggle="modal" data-target="#delete_{{ $labor->labor_guid }}"
                        ...
                        <div class="modal fade" id="modal_{{ $labor->labor_guid }}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    ...
                                    <form action="{{ route('admin.labor_edit') }}" method="POST">
                                        @csrf
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea name="content" class="summernote" required>{!! $labor->content !!}</textarea>
                                            </div>
                                            <div class="form-group">
                                                <label for="recipient-name" class="col-form-label">Görüntülenme
                                                    Sırası</label>
                                                <input type="text" class="form-control"
                                                    value="{{ $labor->queue }}" name="queue">
                                            </div>
                                            <div class="form-group">
                                                <label for="recipient-name" class="col-form-label">Durumu</label>
                                                <select class="form-control" name="status">
                                                    <option value="1" {{ $labor->status == 1 ? 'selected' : '' }}>
                                                        Aktif
                                                    </option>
                                                    <option value="0" {{ $labor->status == 0 ? 'selected' : '' }}>
                                                        Pasif
                                                    </option>
                                                </select>
                                            </div>
                        ...
                        <div class="modal" id="delete_{{ $labor->labor_guid }}" tabindex="-1" role="dialog">
                            <form action="{{ route('admin.labor_delete') }}" method="POST">
                                @csrf
                                <div class="modal-dialog modal-lg" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">Hizmeti Sil</h5>
                                        ...
                                        <div class="modal-body">
                                            <p>{{ $labor->title }} hizmetini silmek istediğinize emin misiniz?</p>
                                        </div>
                                        <input type="hidden" name="labor_guid" value="{{ $labor->labor_guid }}">
                                        <div class="modal-footer">
                    ...
                    <div class="modal fade" id="{{ $data['button']['id'] }}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <form action="{{ route('admin.labor_add') }}" method="POST">
                            @csrf
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
    ...
    <!--end::Body-->
    @if ($labors->lastPage() > 1)
        <div class="card-footer">
            {{ $labors->links('backend.modules.global.paginator', ['paginator' => $labors]) }}
        </div>
    @endif
</div>
@section('js')
