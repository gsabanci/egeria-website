                        ...
                        @foreach($jobapplies as $job)
                            <tr {!! $job->readed === '0' ? 'class="table-primary"' : null !!}>
                                ...
                                <td>
                                    @php
                                        $offices = [];
                                        // job_detail varsa ve offices null değilse dön
                                        if (isset($job->job_detail) && isset($job->job_detail->offices)) {
                                            foreach ($job->job_detail->offices as $of) {
                                                $offices[] = $of->office->title ?? 'Bilinmeyen Ofis';
                                            }
                                        }
                                        $officenames = count($offices) > 0 ? implode(", ", $offices) : 'Ofis Atanmamış';
                                    @endphp
