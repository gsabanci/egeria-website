            ...
            <div class="c-copyright__right">
                ...
                <a href="{{ $s->link }}" target="_blank" aria-label="{{($s->title)}}">
                    <svg class="c-icon__svg c-icon--sm">
                        <use xlink:href="{{ asset('frontend/assets/images/sprite.svg#'.Str::lower($s->title)) }}"></use>
                    </svg>
                </a>
                @endforeach
...
@foreach($policies as $pol)
<div class="modal fade" id="{{ $pol->slug.'-modal' }}" tabindex="-1" role="dialog" aria-labelledby="agreementModal" aria-hidden="true">
